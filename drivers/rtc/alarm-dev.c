...
static void inline devalarms_init(struct alarm_dev_ns *alarm_ns)
{
	int i;
	for (i = 0; i < ANDROID_ALARM_TYPE_COUNT; i++) {
		alarm_init(&alarm_ns->alarms[i].alarm, i, alarm_triggered);
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ANDROID_ALARM_CLEAR(0):
		...
		alarm_try_to_cancel(&alarm_ns->alarms[alarm_type].alarm);
...
from_old_alarm_set:
		...
		alarm_ns->alarm_enabled |= alarm_type_mask;
		alarm_start_range(&alarm_ns->alarms[alarm_type].alarm,
...
static int alarm_release(struct inode *inode, struct file *file)
{
	...
	if (file->private_data != 0) {
		for (i = 0; i < ANDROID_ALARM_TYPE_COUNT; i++) {
			...
			spin_unlock_irqrestore(&alarm_ns->alarm_slock, flags);
			alarm_cancel(&alarm_ns->alarms[i].alarm);
			spin_lock_irqsave(&alarm_ns->alarm_slock, flags);
		}
		if (alarm_ns->alarm_pending | alarm_ns->wait_pending) {
			if (alarm_ns->alarm_pending)
...
static void alarm_triggered(struct alarm *alarm)
{
	...
	struct devalarm *dalarm = container_of(alarm, struct devalarm, alarm);
	struct alarm_dev_ns *alarm_ns = dalarm->alarm_ns;
